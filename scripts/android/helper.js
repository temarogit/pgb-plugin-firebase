...
function addDependencies(buildGradle) {
  ...
  var googlePlayDependency = whitespace + 'classpath \'com.google.gms:google-services:4.2.0\' // google-services dependency from pgb-plugin-firebase';
  var fabricDependency = whitespace + 'classpath \'io.fabric.tools:gradle:1.25.4\' // fabric dependency from pgb-plugin-firebase'
...
function addRepos(buildGradle) {
  ...
  var fabricMavenRepo = whitespace + 'maven { url \'https://maven.fabric.io/public\' } // Fabrics Maven repository from pgb-plugin-firebase'
  var modifiedLine = match[0] + '\n' + fabricMavenRepo;
  ...
  if (allProjectsIndex > 0) {
    ...
    var googlesMavenRepo = whitespace + 'google() // Google\'s Maven repository from pgb-plugin-firebase';
